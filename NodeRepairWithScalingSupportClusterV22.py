...
# version 2.1 branch3 adding insert , delete files and make  node fail, live
# single node scaling required in case of a single node failure. Furthur writes are permitted only when buffer nodes are present through single node scaling.  
# Recovery by Adjustment works only when no buffer nodes are there and when no reads and writes are there.
# Readjust method added
# version 2.2 Cluster concept getting added
# add to branch4
import math
import time
...
class StorageNode:
	...
	def __init__(self, max_blocks, block_size, node_id, node_type):
				...
				self.node_use_status = "Regular" # this can be Regular in normal case or Buffer in case of buffer node or dead node becoming alive
	...
	def make_node_live(self):
		if(self.node_stat == "Live"):
			print "\nNode Already Live\n"
			node_stat = "AlreadyLive"
		else:
			node_stat = "Live"
			self.node_stat = node_stat
			print "\n node live now"
			self.node_use_status = "Buffer"
			for blocks in range(self.max_blocks):
				self.block_stat[blocks] = "Free"
				self.block_data[blocks] = -1
				print "@@@ self.block_data[ "+str(blocks)+"] = " +str(self.block_data[blocks])
				print "@@@ self.block_status[ "+str(blocks)+"] = " +str(self.block_stat[blocks])
		return node_stat
...
class Cluster:
	...
	def __init__(self, max_nodes, datanodes, paritynodes, buffernodes,cluster_no): # max_nodes = datanodes+paritynodes, buffernodes are for failure handling.
		...
		#structure of tbl_node_allocation
		# {Node Name, [total space,used space, Node state -Live \ Failed,[list of file fragments in the node],{data of some failed node - failed node name,[list of failed node data stored]}]
		self.tbl_node_allocation = {} # Tbl4 Node allocation map eg : {DN1, [[Fail],[]], DN2,[50,20,'Live',[A1,B1,C1],{"DN1",[A0,B0]}} 
		self.tbl_file_blocks_list = [] # hold the file block list eg. [A1,A2,PA1,PA2]
		self.tbl_file_stripe_list = [] # hold the file stripe list eg. [[DN1,1],[DN2,1], [PN3,1], [PN4,1]]
		self.tbl_allocation_list = [] # [50,20]
		self.node_fileblk_map = {} # keep track of file blocks in each node
		self.cluster_id = "Clust-"+str(cluster_no)
	...
	def get_tbl_file_blocks_map(self):
		return self.tbl_file_blocks_map
	# set the tbl_file_blocks_map
	def set_tbl_file_blocks_map(self,tbl_file_blocks_map):
		self.tbl_file_blocks_map = tbl_file_blocks_map
	#return set_tbl_file_blocks_map
	def get_tbl_fileblk_nodeblk_map(self):
		return self.tbl_fileblk_nodeblk_map
	#set set_tbl_file_blocks_map
	def set_tbl_fileblk_nodeblk_map(self,tbl_fileblk_nodeblk_map):
		self.tbl_fileblk_nodeblk_map = tbl_fileblk_nodeblk_map
	#return tbl_file_stripe_map 
	def get_tbl_file_stripe_map(self):
		return self.tbl_file_stripe_map 
	#set tbl_file_stripe_map 
	def set_tbl_file_stripe_map(self,tbl_file_stripe_map):
		self.tbl_file_stripe_map = tbl_file_stripe_map 
	#return tbl_node_allocation
	def get_tbl_node_allocation(self):
		return self.tbl_node_allocation
...
class Master:
	...
	def __init__(self, max_nodes, datanodes, paritynodes, buffernodes): # max_nodes = datanodes+paritynodes, buffernodes are for failure handling.
		...
		self.tbl_fileblk_nodeblk_map = {} # Tbl2 File block to node block map eg : {A1, [DN0,1],A2, [DN1,1], PA0,[PN0,1], PA1,[PN1,1]}
		self.tbl_file_stripe_map = {} # Tbl3 File, stripe map eg : A ,[[DN1,1],[DN2,1], [PN3,1], [PN4,1]]
		self.tbl_node_allocation = {} # Tbl4 Node allocation map eg : {DN1, [[Fail],[]], DN2,[50,20,'Live',[A1,B1,C1]
		self.tbl_file_blocks_list = [] # hold the file block list eg. [A1,A2,PA1,PA2]
		self.tbl_file_stripe_list = [] # hold the file stripe list eg. [[DN1,1],[DN2,1], [PN3,1], [PN4,1]]
		self.tbl_allocation_list = [] # [50,20]
	...
	# return active cluster id
	def get_active_cluster_id(self,cluster_no):
		return self.clusters[cluster_no].cluster_id 
	...
	def write_file(self, file_name, file_data=[]):  
		...
		for nodes in range(no_of_nodes): # taking count of live nodes
			if(self.clusters[self.active_cluster].storage_node[nodes].node_stat == "Live"):
				live_node_count += 1
		if(live_node_count < self.max_nodes): # checking for sufficient number of live nodes
		...
		if(status !="No Duplicates"):		 #  
			...
			if not (self.clusters[self.active_cluster].storage_node[self.max_nodes-1].give_list_free_blocks()): # checking to see the nodes are full and if so scale
			...
			for nodes in range(start_node,end_node): # trying to write the data to the node blocks.
				#for blocks in range(storage_node[nodes].max_nodes)
					...
					if(self.clusters[self.active_cluster].storage_node[nodes].node_stat == "Failed"): # added to check if the node is a failed node and skip that node
						...
						continue
					if(self.clusters[self.active_cluster].storage_node[nodes].node_use_status == "Buffer"):
		...
		if(file_name not in self.tbl_file_blocks_map):
				...
				if(filefrags in self.tbl_fileblk_nodeblk_map):
					...
					if(free_node not in self.tbl_node_allocation):
						...
						if(self.tbl_node_allocation[free_node][1] == self.tbl_node_allocation[free_node][0]): 
							...
							self.clusters[cluster].storage_node[free_node_num].block_stat[free_block] = "Free"
			...
			if(self.is_cluster_free(cluster) == True):
				...
				if(self.cluster_count==1):
					print "No scaling down required"
				else:
					print "going to scale down"
			else:
	...
	def display_tables(self):
		...
		print "\n ****** tbl_file_blocks_map : "+str(self.clusters[self.active_cluster].get_tbl_file_blocks_map())
		...
		print"\n tbl_cluster_file_info : "+ str(self.tbl_cluster_file_info)
	...
	def make_node_live(self, node):
		if(node >= 0 and node < self.max_nodes):
			status = self.clusters[self.active_cluster].storage_node[node].make_node_live()
			if( status == "Live"):
				self.tbl_node_allocation[self.clusters[self.active_cluster].storage_node[node].node_id][2] = "Live"
				self.tbl_node_allocation[self.clusters[self.active_cluster].storage_node[node].node_id][3] = []
				return "Live"
			elif(status == "AlreadyLive"):
				return "AlreadyLive"
	...
	# returns the cluster number given the cluster id
	def give_cluster_number(self,cluster_id):
		clusterno = cluster_id.split('-')
		return int(clusterno[1])

	# return number of used blocks
	def give_number_of_used_blocks(self,node_id,cluster_no):
		number_used_blocks = 0
		node_number = self.give_node_number(node_id)
		#node_number = self.nodeid_nodenumber_map[node_id]
		max_blocks = self.clusters[cluster_no].storage_node[node_number].max_blocks
		for used_blocks in range(max_blocks):
			if(self.clusters[self.active_cluster].storage_node[node_number].block_stat[used_blocks] == "Allocated"):
				number_used_blocks += 1
		return number_used_blocks
	#return the number of used nodes in a cluster 
	def is_cluster_free(self,cluster_no):
		total_used_blks = 0
		for nodes in range(len(self.clusters[cluster_no].storage_node)):
			node_id = self.clusters[cluster_no].storage_node[nodes].node_id
			used_blocks = self.give_number_of_used_blocks(node_id,cluster_no)
			total_used_blks += used_blocks
			max_blocks = self.clusters[cluster_no].storage_node[nodes].max_blocks
		if(total_used_blks == (self.max_nodes*max_blocks) or (total_used_blks > 0)):
			print "\n Cluster not free"
			return False
		elif(total_used_blks == 0):
			print "\n CLuster Free"
			return True
	...
	def start_recovery_adjust_mode(self, node_id):
		...
		reqd_no_blks = reqd_size/(self.clusters[self.active_cluster].storage_node[nodenumber].block_size)
		temp = 0
		available_space = 0
		nxtblk = []
			...
			for nodes in free_block_count:
				...
				for blk in range(min(temp, reqd_no_blks)):
					...
					if(self.clusters[self.active_cluster].storage_node[temp_node_id].write_to_node_stripe(data,nxtblk[1]) == "Done"):
						...
						reqd_no_blks -= 1
						free_block_count[nodes][0] -= self.clusters[self.active_cluster].storage_node[temp_node_id].block_size 
...
while((exit=="T") or (exit == "t")):
	...
	if (answer == "Y" or answer == "y"):
		...
		if(sb == "Y" or sb == "y") :
			...
			if(ma.make_node_fail(int(node_to_fail))== "Already Failed"):
				print "\n Can not fail an already failed node"
		else:
			node_to_fail = raw_input("NOt SB Enter Node number to fail : ")
			if(ma.make_node_fail(int(node_to_fail))== "Already Failed"):
				print "\n Can not fail an already failed node"
	live = raw_input("\n Do you want a node live : (Y/N) ")
	if(live == "Y" or live == "y"):
		node_to_live = raw_input("Enter the node to be live : ")
		if(ma.make_node_live(int(node_to_live))== "AlreadyLive"):
